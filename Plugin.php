...
<?php
		...
		$qiniuprefix = new Typecho_Widget_Helper_Form_Element_Hidden('qiniuprefix',NULL,'HSgallery/{year}/{month}/{day}/');
	 ...
	 * @return void
	 */
	public static function initQiniuSDK($accesskey,$secretkey)
	{
	...
	public static function initQiniu($accesskey, $sercetkey)
    {
        require_once("cloud/qiniu/vendor/autoload.php");
	...
	public static function filedata($path = NULL,$url = NULL)
	{
		...
		if (strpos($url,$options->siteUrl)===false) {
			...
			if ($settings->storage=='qiniu') {
				$savepath = preg_replace(array('/\{year\}/', '/\{month\}/', '/\{day\}/'), array($date->year, $date->month, $date->day), $settings->qiniuprefix);
				$prefix = ($path)?$path:$savepath;
				$fileurl = Typecho_Common::url($prefix,$settings->qiniudomain);
				$filedir = ($path)?substr($path,1):$savepath;
			}
	...
	public static function filelist()
	{
		...
		//获取七牛列表
		if ($settings->storage=='qiniu') {
			self::initQiniuSDK($settings->qiniuaccesskey,$settings->qiniusecretkey);
	...
	public static function form($action = NULL,$render = '1')
	{
		...
		$qiniuprefix = new Typecho_Widget_Helper_Form_Element_Text('qiniuprefix',
			NULL,$settings->qiniuprefix,_t('路径前缀'),_t('注意开头不要加/号，结尾要加/，可选参数：{year} 年份、{month} 月份、{day} 日期'));
...
';
	...
	public static function footlink()
	{
		...
		if ($widget->is(''.$istype->index.'')||$widget->is(''.$istype->archive.'')||$widget->is(''.$istype->post.'')||$widget->is(''.$istype->page.'')) {
			$links = '
<script data-no-instant type="text/javascript" src="'.$hsurl.'js/'.$settings->mode.'"></script>';
			$links.='
...
//]]>
	...
	public static function uploadhandle($file)
	{
		...
		//上传路径
		$settings = Helper::options()->plugin('HighSlide');
		$savepath = self::filedata()->dir;
		if ($settings->storage=='local'&&!is_dir($savepath)) {
			if (!self::makehsdir($savepath)) {
				return false;
			}
		}
		...
		//七牛上传
		if ($settings->storage=='qiniu') {
			$token = self::initQiniu($settings->qiniuaccesskey,$settings->qiniusecretkey)->uploadToken($settings->qiniubucket);
	        // 上传
	        $uploadMgr = new Qiniu\Storage\UploadManager();
	        list($result, $error) = $uploadMgr->putFile($token, $imgpath, $filename);
	        echo "\n====> putFile result: \n";
	        if ($error !== null)
	        {
	            var_dump($error);
	...
	public static function crophandle($imgname,$width,$height,$xset,$yset,$path = NULL,$url = NULL)
	{
		...
		if (strpos($url,$options->siteUrl)===false) {
			...
			if ($settings->storage=='qiniu') {
		        ...
		        // 上传
		        $uploadMgr = new Qiniu\Storage\UploadManager();
		        list($result, $error) = $uploadMgr->putFile($token, $thumbpath, $thumbfile);
		        if ($error !== null)
		        {
		            return false;
		        }
